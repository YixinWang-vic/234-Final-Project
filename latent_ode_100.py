...
# if use set CUDA seed
if torch.cuda.is_available():
    torch.cuda.manual_seed(123)
    torch.cuda.manual_seed_all(123)  # 如果使用多个 GPU
...
parser.add_argument('--niters', type=int, default=1000) # to save time, I use 1000 instead of 10000
...
if __name__ == '__main__':
    ...
    if args.visualize:
        with torch.no_grad():
            ...
            ts_pos = np.linspace(0., 2. * np.pi, num=2000) #2000 originally
            ts_neg = np.linspace(-np.pi, 0., num=2000)[::-1].copy() #2000 originally
        ...
        plt.legend()
        plt.savefig('/workspace/figure-100.png', dpi=500)
        plt.show()
        print('Saved visualization figure at {}'.format('/workspace/figure-100.png'))
